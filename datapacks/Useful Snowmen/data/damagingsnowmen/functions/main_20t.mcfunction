...
# get item count of rock flint ### MODIFIED BY CAPNAPU
execute as @e[type=item,nbt={Item:{id:"extcaves:rock_flint"}}] store result score @s cc.sg.counter run data get entity @s Item.Count
# rock flint exists, now test for snow around ### MODIFIED BY CAPNAPU
execute as @e[type=item,nbt={Item:{id:"extcaves:rock_flint"}},scores={cc.sg.counter=2..}] at @s run function damagingsnowmen:buttontest_20t
